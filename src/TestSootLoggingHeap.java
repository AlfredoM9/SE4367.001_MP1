            ...
            while(it.hasNext()){
                ...
                if (stmt.containsFieldRef()) {
                    if (stmt instanceof AssignStmt){
                        // Get AssignStmt object
                        AssignStmt assignStmt = (AssignStmt) stmt;

                        // Get the isWrite from the AssignStmt object
                        boolean isWrite = assignStmt.getLeftOp() instanceof FieldRef;

                        // Get the value
                        Value value = assignStmt.getRightOp();

                        // Get other parameters
                        SootField object = stmt.getFieldRef().getField();
                        String name = stmt.getFieldRef().getField().getName();
                        boolean isStatic = stmt.getFieldRef().getField().isStatic();

                        // Call method to print
                        Log.logFieldAcc(object, name, isStatic, isWrite, value);
                    }
                }
            }
        }
